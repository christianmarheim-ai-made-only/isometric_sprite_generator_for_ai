...
| ADR-0038 | Front portrait render for the character screen — a SEPARATE front/eye-level orthographic still (idle pose, STUDIO light, 512²) emitted as `portrait.png` + a manifest `portrait` block; additive, leaves locked game_iso_v1 gameplay untouched | **proposed** |
| ADR-0039 | Tiered-terrain bake contract (walkable height-tiers): flat-top tile stays engine-elevated (no per-tier bake); only the VERTICAL pieces (cliff_face/ramp/stair) are baked at 30°; `env_asset_v2` gains a `tier` block; Gate-3 foreshortening probe extends to tier geometry (the irreversible-mistake gate). Paired pipeline ADR | **proposed (gated on engine owner)** |
